# gigachess-zobrist-and-moves2 Specification
...
- **WHEN** a game encoded as a `Uint16Array` (or `Uint8Array`) is replayed via `Chess.fromMoves2(buffer)` or `Board.fromMoves2(buffer)`
