# ADR-015: Rename the library distribution purechess → turbochess

**Date:** 2026-08-31
**Status:** Accepted
**Change:** `turbochess-adopt` (tasks 1.1–1.2)

> Note: this ADR is numbered **015** — the change tasks say `013-turbochess-rename.md`,
...
1. **Rename the npm package to `turbochess`** (turbo = speed). Verified free:
   `npm view turbochess` → 404 on 2026-08-31 (`naming-report.md`); fallback
...
2. **`purechess` stays as a one-release alias**: the `alias/purechess/`
   scaffold publishes a thin re-export package that depends on `turbochess`
...
3. **Exports map stays the same shape** (`.` `./core` `./pgn` `./chess960`
   ...
   `turbochess/chessops/chess` as pure specifier changes with zero logic
   changes.
4. Docs, README, bench harness labels and source headers update `purechess`
   → `turbochess` with the alias note. Historical artifacts keep their
...
- `npm publish --access public` reserves `turbochess`.
- **Alias removal plan:** after one minor release of `turbochess`, stop
  publishing `purechess`, mark `purechess` `deprecated` on npm with
  `"deprecated": "renamed to turbochess"` in its `package.json`, and delete
  `alias/purechess/`.
- The workstation (separate repo) swaps `chessops` → `turbochess/chessops`
