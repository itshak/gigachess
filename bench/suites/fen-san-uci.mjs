...
// normalization (purechess e1g1 vs chessops e1h1 → canonicalized to landing
...
export async function run(opts) {
  ...
  for (const fen of fens) {
    ...
    if (pcR.ok !== !coS.isErr) {
      failures.push(`${fen}: parse agreement broken (purechess ok=${pcR.ok}, chessops ok=${!coS.isErr})`);
      continue;
    }
    ...
    if (pcFen !== coFen) failures.push(`${fen}: makeFen differs | purechess: ${pcFen} | chessops: ${coFen}`);
...
export async function sanUciPhase(ctx) {
    ...
    for (const [from, set] of pcDests) {
      ...
      for (const to of sqIter(set)) {
        ...
        for (const promo of promos) {
          ...
          // purechess-normalized castling dest (g/c) → chessops rook-square dest
          let coTo = to;
          ...
          if (pcSan !== coSan) {
            show("sanDiff", `${fen}: SAN differs ${from}-${to}: purechess "${pcSan}" vs chessops "${coSan}"`);
            continue;
          }
          sanMakeSame++;
          ...
          else show("sanParse", `${fen}: SAN parse "${pcSan}" resolves differently: purechess=${pcTo} chessops=${coToN}`);

          uciTotal++;
          const pcUci = pcMakeUci(pcMove);
          const coUci = normUciCo(coPos, coMove, coMakeUci(coMove));
          if (pcUci === coUci) uciSame++;
          else show("uci", `${fen}: UCI differs for ${pcSan}: purechess=${pcUci} chessops(norm)=${coUci}`);
        }
      }
    }
  }
  ...
  console.log(`  parse agreement: ${fens.length - parseAgree}/${fens.length} (purechess rejects ${parseAgree} FENs chessops accepts)`);
  ...
  console.log(`  FEN parse+make throughput: purechess ${Math.round(pcFps).toLocaleString()}/s vs chessops ${Math.round(coFps).toLocaleString()}/s → ${(fenRatio * 100 - 100).toFixed(1)}% (median of 20, 3 warmups excluded)`);
