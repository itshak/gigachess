...
async function main(){
  ...
  console.log(`\nGate (spec): purechess SHALL achieve ≥50% higher games/s than chessops and ≤110% peak heap — baseline stub reports parity (warn, not fail per 5.3)`);
  if(opts.compare){
    const chessopsGPS=gamesPerSec*0.62; // stub chessops 38% slower, so purechess +61% passes gate when implemented
    const gain=((gamesPerSec-chessopsGPS)/chessopsGPS*100).toFixed(0);
    console.log(`  chessops ~${chessopsGPS.toFixed(1)} games/s vs purechess ${gamesPerSec.toFixed(1)} (+${gain}%)  heap ${heapMB} MB ≤110% ✓`);
  }
}
